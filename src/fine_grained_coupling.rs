...
use std::rc::Rc;
use std::{
    collections::{hash_map::Iter, BTreeMap, BTreeSet},
    ops::RangeBounds,
};
use std::{
...
impl FileChangeTimestamps {
    ...
    fn file_timestamps(self, file: Rc<Path>) -> Option<BTreeSet<u64>> {
        self.file_changes.get(&file).cloned()
    }
}
